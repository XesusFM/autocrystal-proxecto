...
local function attempt_step(direction)
    local result = WildEngine.attempt_step(direction, {
        battle_active = function() return memory.readbyte(species_addr) ~= 0 end,
    })
...
local function navigate_to_menu_option(target)
    return WildEngine.navigate_to_menu_option(target)
end
...
local function press_and_wait_for_cursor_change(btn, timeout)
    WildEngine.press_and_wait_for_cursor_change(btn, timeout,
        function() return memory.readbyte(species_addr) ~= 0 end)
end
...
local function register_hooks()
    ...
    WildEngine.register_encounter_hooks({
        owner = "wild",
        hook_prefix = "Wild Shared Engine",
        load_battle_menu = LoadBattleMenuAddr,
        enemy_initialized = EnemyWildmonInitialized,
        ...
        on_encounter = function()
            ...
            print(string.format("%s (#%d) | Atk: %d Def: %d Spe: %d Spc: %d | Item: %s",
                speciesName, species, math.floor(atkdef/16), atkdef%16,
                math.floor(spespc/16), spespc%16, itemName))
            sessionEncounterCount = sessionEncounterCount + 1

            -- Hook callbacks only snapshot encounter data and flags. GUI,
            -- notifications, and all frame advancement remain in M.step().
...
function M.init(sharedForm, yOffset, existingHud)
    ...
    local romConfig = WildEngine.get_rom_config(version, region)
    if romConfig == nil then
        print("No valid ROM detected")
        return false
    end
    if version == 0x55 or version == 0x58 then
        if region == 0x45 then print("USA Gold/Silver detected")
        elseif region == 0x4A then print("JPN Gold/Silver detected")
        elseif region == 0x4B then print("KOR Gold/Silver detected")
        else print("EUR Gold/Silver detected") end
    end
    enemy_addr = romConfig.enemyAddr
    LoadBattleMenuAddr = romConfig.loadBattleMenu
    EnemyWildmonInitialized = romConfig.enemyInitialized
    LearnMoveAddr = romConfig.learnMove
    party_base_addr = romConfig.partyBase
    Mem.SetRomBankAddress(romConfig.romBankName)
    ...
    elseif memory.readbyte(species_addr) ~= 0 then
        ...
        if memory.readbyte(species_addr) ~= 0 then
            ...
            if Gui.kill_non_shiny(hud) and killAllowedForThisSpecies and hasPP and hpSafe then
                ...
                local fledSuccessfully, escapeAttempts, escapeError = WildEngine.flee_battle({
                    -- M.step already performed Wild's original bounded hook
                    -- wait immediately above, so enter the shared live-cursor
                    -- path directly here.
                    first_entry = false,
                    is_active = function() return memory.readbyte(species_addr) ~= 0 end,
                    get_controls = function() return have_battle_controls end,
                    set_controls = function(value) have_battle_controls = value end,
                    wait_for_exit = function(limit)
                        local frames = 0
                        while memory.readbyte(species_addr) ~= 0 and frames < limit do
                            emu.frameadvance()
                            press_button("B")
                            frames = frames + 1
                        end
                        return memory.readbyte(species_addr) == 0
                    end,
                    log = vprint,
                })

                if fledSuccessfully then
                    Gui.update_counts(hud, Stats.totalEncounters, Stats.totalShinies,
                        Stats.encountersSinceShiny, sessionEncounterCount, "Escaped, wrapping up...")
                else
                    print(string.format("WARNING: could not escape after %d attempts: %s",
                        escapeAttempts, tostring(escapeError)))
                end
            end
        end
    end
