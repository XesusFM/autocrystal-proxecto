...
--   modules/wild.lua, wild_engine.lua, gui_module.lua, campaign.lua, etc.
...
local MODULES = {
    ...
    { name = "Route Campaign", module = "campaign", available = true, art = "launcher_art1.png" },
}
...
local sharedWildEnginePrepared = false
local function require_launcher_module(name)
    if (name == "wild" or name == "campaign") and not sharedWildEnginePrepared then
        -- BizHawk preserves package.loaded when launcher.lua is stopped and
        -- re-added. Invalidate both consumers exactly once so Wild and Campaign
        -- always receive the same fresh shared engine in this launcher run.
        for _, package_name in ipairs({
            "wild", "wild_engine", "campaign", "campaign_capture",
            "campaign_gui", "campaign_store",
        }) do
            package.loaded[package_name] = nil
        end
        sharedWildEnginePrepared = true
    end
    return require(name)
end
...
while true do
    ...
    if startRequestedEntry ~= nil then
        ...
        elseif not chosen.available then
            ...
            if initializedModules[chosen.module] == nil then
                local mod = require_launcher_module(chosen.module)

                -- Long-running modules may cooperatively yield from bounded
                -- frame waits as soon as the Stop callback records intent.
                -- The callback still performs no gameplay or frame advance.
                if mod.set_stop_checker then
                    mod.set_stop_checker(function() return stopRequested end)
                end
            ...
            if mod ~= nil then
                if loadedModule ~= nil and loadedModule ~= mod and loadedModule.on_switch_away then
                    loadedModule.on_switch_away()
                end
                loadedModule = mod
    ...
    if stopRequested then
        stopRequested = false
        running = false
        if loadedModule ~= nil and loadedModule.on_stop then
            loadedModule.on_stop()
        end
        ActiveModuleName = nil
