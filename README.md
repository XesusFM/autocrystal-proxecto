...
- [Modules](#modules)
  ...
  - [Route Campaign Module](#route-campaign-module-beta)
- [FAQ](#faq)
...
### Route Campaign Module 🧪
- Persistent multi-stage shiny target lists
- Automatic non-shiny fleeing and Master Ball capture
- Integrated walking-route recorder with RAM-verified checkpoints
- Automatic travel to the next stage after completing every target in a zone
- Persistent profiles, progress, extra-shiny history, and Discord notifications
- English Pokemon Crystal Version (USA, Europe) only in the first release
...
## Route Campaign Module (Beta)

Route Campaign chains several wild-encounter hunting zones into one persistent
campaign. Each stage has a list of shiny species. The bot hunts at the saved
anchor, flees from non-shinies, catches every shiny with a Master Ball, and
walks a recorded route to the next stage after all targets are captured.

> **Beta safety warning:** the persistence and encounter logic have automated
> tests, but the complete PACK/capture sequence still requires manual validation
> in BizHawk with the English USA/Europe Crystal ROM. Back up your battery save
> and test
> with a disposable setup before leaving this mode unattended around a real
> shiny.

For the first manual validation, load `modules/diagnose_campaign_capture.lua`
directly in BizHawk's Lua Console and follow the instructions at the top of the
file. It only reads and prints the campaign-related RAM values; it never presses
buttons or writes game memory.

### Requirements

- English **Pokemon Crystal Version (USA, Europe)** (`0x54/0x45`). Gold,
  Silver, Japanese Crystal, and the other European localizations are rejected.
- One Master Ball for every shiny that may appear, including duplicates and
  species outside the current target list.
- If the party is full, the active PC box must have room. The game refuses to
  use the Master Ball when both are full; the campaign detects the unconsumed
  Ball, pauses, and leaves the battle for manual intervention.
- A walkable pair of tiles at every hunting anchor.
- Transitions that require only directional walking. Surf, Cut, NPC dialogue,
  trainer battles, PC use, shopping, and recorded A/B/Start/Select actions are
  not supported in v1.

### Creating a campaign

1. Select **Route Campaign** in the launcher and click **Start**. Start only
   activates the campaign editor; hunting does not begin yet.
2. Enter a profile name and click **Create**.
3. Enter the first stage name and a comma-separated target list. Species names
   and National Dex IDs are accepted, for example `Pidgey, Rattata, 163`.
4. Click **Add stage**, stand on the tile where the bot should hunt, select that
   stage, and click **Set current anchor**.
5. Add the next stage, then select the previous stage and click **Record route**.
6. Walk manually to the next hunting tile. The recorder stores each successful
   direction together with its starting and resulting map coordinates. Click
   **Stop + save** at the destination; that tile becomes the next stage's
   anchor.
7. Repeat for additional stages. The final stage needs an anchor but no outgoing
   route.
8. Click **Run campaign**. A profile is runnable only when every stage has at
   least one target and an anchor, and every non-final stage has a route.

During playback, every step must match its recorded map and coordinates. Wild
...
battle-menu waits, escape attempts, and capture dialogue check the pending stop
request after every emulated frame, release all joypad input, and return control
to the launcher without waiting for the current watchdog timeout.

Wild Encounters and Route Campaign now consume the same `wild_engine.lua`
implementation for ROM configuration, hooks, RAM-confirmed movement, button
timing, cursor navigation, and the five-attempt escape loop. Encounter creation
is hook-driven; transient switchable-WRAM values cannot invent battles, and a
missed `LoadBattleMenu` hook falls back to the same bounded live-cursor path as
Wild. The Master Ball action additionally verifies the Balls-pocket state,
selected item, the RAM-confirmed `USE/QUIT` submenu before accepting `USE`,
successful-capture species, Ball consumption, and stable battle exit.
If BizHawk drops an `A` edge, the engine retries once only while RAM confirms
that Master Ball or the default `USE` entry is still selected; it never adds an
unconditional extra `A` after the menu has changed.

An objective is completed only after the species, Master Ball consumption, and
battle exit are all confirmed. A duplicate or non-target shiny is still captured
but is recorded as an extra. Shinies encountered while traveling are always
extras and never complete the next stage early. The campaign stops and notifies
after the final target list is complete.

Profiles and progress are written atomically under `user_data/`. That directory
is ignored by Git. Stage structure is locked after progress exists; use the
two-click **Reset progress** action before changing target lists or stage order.
